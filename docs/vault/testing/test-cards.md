...
- **Actual builders (current reality):** the Pi's claude -p builds most feature cards; Hermes builds select cards (CM-002 done, CM-014 in flight). Tester = the other builder: Hermes tests Pi-built features (T03/T04/T06/T10), Pi tests Hermes-built features (T05). This supersedes plan-v2's assignee labels for execution — plan-v3 will re-state ownership (so CM-T06 testing the CM-030 series is cross-assigned: Hermes tests, the Pi builds).
