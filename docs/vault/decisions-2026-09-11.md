...
- **Agent sync rule (2026-09-11, per Ram's directive after the CM-014/verdict drift):** Ram's decisions are recorded ONCE, in `docs/vault/decisions-*.md`, by the orchestrator holding the channel he answered in. Every other agent syncs FROM that doc and never re-interprets secondhand answers. Disagreements go mailbox-first before any doc lands. (Proposed by Hermes, mailbox msg-000055; confirmed by AmosBot. PR #14 implements this for the F6/D3 storage rows.)
- **CM-014 engine constants are provisional:** the numbers landed in PR #15 (CP rate ~0.133/min, threshold 0.85, conversion ~12%, home advantage +15%) are audit-derived placeholders pending CM-R03 harness calibration. The engine contract is determinism — seeded PRNG (Mulberry32) + fixed step order; constants live in tunable config and are not spec. PR #15 must pass the CM-T determinism gate before merge.